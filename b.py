...
class Xingda():
	...
	def connect_server(self, server_url, username, password):
	...
	def get_all_view_names(self, connected_server):
		#x = connect_server()
		all_view_names = []
	...
	def get_a_view_jobs(self, connected_server, view_name):
	...
	def job_d(self, server, jobs):
		for job in jobs:
			each_job = server.get_job_info(job)
			# return each_job
			# print each_job['name']

	def job_dd(self, job, server, output):
		# lock.acquire()
		each_job = server.get_job_info(job)
...
server = test.connect_server(config['server_url'], config['username'], config['password'])
all_view_names = test.get_all_view_names(server)
# print all_view_names
jobs =  test.get_a_view_jobs(server, 'tab 1');
...
processes = [mp.Process(target=test.job_dd, args=(job, server, output)) for job in jobs]
